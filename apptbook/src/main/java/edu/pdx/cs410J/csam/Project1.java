  ...
  public static void main(String[] args) {
    ...
    switch(args.length) {
      ...
      case (7):
        ...
        appointment.setAppointment(args[descInd], args[dateInd], args[timeInd]);

        if (args[0].equals("-README")) {
          printReadme();
        }
        else if (args[0].equals("-print")) {
          System.out.println("Description: " + args[descInd]);
        }
    ...
    for(int i = 0; i < 2; ++i) {
      ...
      if (args[dateInd].trim().length() < 8 || args[dateInd].trim().length() > 10) {
        System.out.println("Invalid " + startOrEnd + " date (Format is mm/dd/yyyy)");
        System.exit(1);
      }

      //Month is not represented by a number
      if (!Character.isDigit(args[dateInd].trim().charAt(0))) {
        System.out.println("Invalid " + startOrEnd + " date (Month is not represented by a number)");
        System.exit(1);
      }
      ...
      if (Character.isDigit(args[dateInd].trim().charAt(0)) && Character.isDigit(args[dateInd].trim().charAt(1))) {
        ...
        if (Character.getNumericValue(args[dateInd].trim().charAt(0)) > 1) {
          System.out.println("Invalid " + startOrEnd + " date (First digit of month is not 0 or 1)");
          System.exit(1);
        }
      ...
      if (Character.isDigit(args[dateInd].trim().charAt(dayInd)) && Character.isDigit(args[dateInd].trim().charAt(dayInd + 1))) {
        ...
        if (Character.getNumericValue(args[dateInd].trim().charAt(dayInd)) > 3) {
          System.out.println("Invalid " + startOrEnd + " date (First digit of day is not 0, 1, 2, or 3)");
          System.exit(1);
        }
        //First digit of day is 3, second digit is not 0 or 1
          ...
          System.out.println("Invalid " + startOrEnd + " date (First digit of day is 3, second digit is not 0 or 1)");
          System.exit(1);
        }
        //Day is 00
          ...
          System.out.println("Invalid " + startOrEnd + " date (Day is 00)");
          System.exit(1);
        }
        //Day is Feb. 30
          ...
          System.out.println("Invalid " + startOrEnd + " date (There is no Feb. 30)");
          System.exit(1);
        }
        //Day is Feb. 31
        else if(Character.getNumericValue(args[dateInd].trim().charAt(dayInd)) == 3 && Character.getNumericValue(args[dateInd].trim().charAt(dayInd + 1)) == 1
                && ((Character.getNumericValue(args[dateInd].trim().charAt(0)) == 0 && Character.getNumericValue(args[dateInd].trim().charAt(1)) == 2)
                || Character.getNumericValue(args[dateInd].trim().charAt(0)) == 2)) {
          System.out.println("Invalid " + startOrEnd + " date (There is no Feb. 31)");
          System.exit(1);
        }
        //Day is Apr. 31
        else if(Character.getNumericValue(args[dateInd].trim().charAt(dayInd)) == 3 && Character.getNumericValue(args[dateInd].trim().charAt(dayInd + 1)) == 1
          ...
          System.out.println("Invalid " + startOrEnd + " date (There is no Apr. 31)");
          System.exit(1);
        }
        //Day is Jun. 31
        else if(Character.getNumericValue(args[dateInd].trim().charAt(dayInd)) == 3 && Character.getNumericValue(args[dateInd].trim().charAt(dayInd + 1)) == 1
          ...
          System.out.println("Invalid " + startOrEnd + " date (There is no Jun. 31)");
          System.exit(1);
        }
        //Day is Sep. 31
        else if(Character.getNumericValue(args[dateInd].trim().charAt(dayInd)) == 3 && Character.getNumericValue(args[dateInd].trim().charAt(dayInd + 1)) == 1
      ...
      if (!Character.isDigit(args[dateInd].trim().charAt(yearInd)) || !Character.isDigit(args[dateInd].trim().charAt(yearInd + 1))
        ...
        System.out.println("Invalid " + startOrEnd + " date (Year is not represented by a number)");
        System.exit(1);
      }
      ...
      if(Character.getNumericValue(args[dateInd].trim().charAt(dayInd)) == 2 && Character.getNumericValue(args[dateInd].trim().charAt(dayInd + 1)) == 9
        ...
        System.out.println("Invalid " + startOrEnd + " date (Day is Feb. 29, but year is not a leap year)");
        System.exit(1);
      }

      //Time is too long or too short
      if (args[timeInd].length() < 4 || args[timeInd].length() > 5) {
        System.out.println("Invalid " + startOrEnd + " time (Format is hh:mm)");
        System.exit(1);
      }

      //Hour is not represented by a number
      if (!Character.isDigit(args[timeInd].trim().charAt(0))) {
        System.out.println("Invalid " + startOrEnd + " time (Hour is not represented by a number)");
        System.exit(1);
      }
      ...
      if (Character.isDigit(args[timeInd].trim().charAt(0)) && Character.isDigit(args[timeInd].trim().charAt(1))) {
        ...
        if (Character.getNumericValue(args[timeInd].trim().charAt(0)) > 2 || Character.getNumericValue(args[timeInd].trim().charAt(0)) <= 0) {
          System.out.println("Invalid " + startOrEnd + " time (First digit of hour is not 1 or 2)");
          System.exit(1);
        }
        ...
        //Hour is larger than 24
          ...
          System.out.println("Invalid " + startOrEnd + " time (Hour is larger than 24)");
          System.exit(1);
        }
      ...
      if (!Character.isDigit(args[timeInd].trim().charAt(minInd)) || !Character.isDigit(args[timeInd].trim().charAt(minInd + 1))) {
        System.out.println("Invalid " + startOrEnd + " time (Minute is not represented by a number)");
      ...
      //Minute is larger than 60
        ...
        System.out.println("Invalid " + startOrEnd + " time (Minute is larger than 60)");
        System.exit(1);
      }
      ...
      //Hour is 24 and minute is not 00
        ...
        System.out.println("Invalid " + startOrEnd + " time (Minute exceeds 24:00)");
        System.exit(1);
      }
